...
const TrainingBudgetRequestForm = ({ buttonName, selectedObject, handleIsOpen }) => {
  const [loading, setLoading] = useState(false);
  
  const [formData, setFormData] = useState({
    id: selectedObject?.id || '',
    eid: selectedObject?.eid || localStorage.getItem('userId') || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Form validation
      if (!formData.eid) {
        toast.error("Employee ID is missing. Please make sure you're logged in.");
        setLoading(false);
        return;
      }
      ...
      if (selectedObject) {
        ...
        toast.success("Training budget request updated successfully");
      } else {
      ...
      console.error("Error saving training budget request:", error);
      
      let errorMessage = "Error submitting budget request. Please try again.";
    ...
    <div>
      <form onSubmit={handleSubmit} className="p-4 md:p-5">
        <div className="grid gap-4 mb-4 grid-cols-2">
          ...
          <div className="col-span-2">
            ...
            <textarea
              ...
              rows="3"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
              placeholder="Enter reason for request (e.g., Python Certification Course)"
              required
            />
          </div>
          ...
          {/* Status is only editable by admin, not visible for regular employee form */}
          ...
          {/* Granted date is only editable by admin, not visible for regular employee form */}
          {localStorage.getItem('userRole') === 'admin' && (
            <div className="col-span-2 sm:col-span-1">
          ...
          <div className="col-span-2">
            ...
            <input
              ...
              onChange={handleChange}
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
              placeholder="Enter document URL (http://example.com/docs/1.pdf)"
            />
          </div>
        </div>
        ...
        <div className="flex justify-between mt-4">
          ...
          >
            {loading ? 'Processing...' : buttonName || 'Submit Request'}
          </button>
          <button
            type="button"
...
export default TrainingBudgetRequestForm;
